...
def rm(command):
    ...
    fileName = words[1]
    deleteBLOB(current_dir=globals.current_dir, fileName=fileName)
    globals.history = globals.history.append({'success': command, 'failure': '-'}, ignore_index=True)
...
def create(command):
    ...
    data = raw_input()
    createBlob(filePath=globals.current_dir, fileName=fileName, input=data)
    globals.history = globals.history.append({'success': command, 'failure': '-'}, ignore_index=True)

def merge_files(words):
    logging.info('merging files ' + words[2] + words[3])

    if retrieveBLOB(globals.current_dir + words[1]) is not None:
        logging.error("a file with the same name already exists")
        return
    try:
        ...
        createBlob(filePath=globals.current_dir, fileName=words[1], input=data)
    except:
...
def addFile(command):
    ...
    if os.path.exists("/home/ioana/PycharmProjects/filesystem/resources/"+fileName):
        input = "/home/ioana/PycharmProjects/filesystem/resources/"+fileName
        copyFile(globals.current_dir,fileName,input)
